namespace :db do
  ...
  task :upload_to_s3 do
    ...
    puts "Uploading './#{db_file_name}' to S3...".gray
    Aws::S3::Resource.new
      .bucket(AWS_BUCKET)
  ...
  task :download_from_s3 do
    ...
    if ::File.exist?("./#{db_file_name}") && ::File.ctime("./#{db_file_name}") > remote_db_last_modified
      ...
      unless ["y", "yes"].include?($stdin.gets.chomp.downcase)
        puts "Skipping remote state download".green
        exit 0
      end
    end

    puts "Downloading './#{db_file_name}' from S3...".gray
  ...
  task :upload_to_pcloud do
    ...
    puts "Uploading '#{LOCAL_DB_FILENAME}' to pCloud...".gray
    Pcloud::File.upload(
      ...
      .each do |state_file|
        ...
        if ::File.exist?(filename) && ::File.ctime(filename) > state_file.created_at
          ...
          unless ["yes", "y"].include?($stdin.gets.chomp.downcase)
            puts "Skipping download of '#{filename}'...".gray
            next
          end
        end
        ...
        ::File.open(filename, "w") do |file|
          file.binmode
          puts "Downloading '#{filename}' from pCloud...".gray
  ...
  task :report_totals_to_sns do
    ...
    puts "Kanji report sent to SNS".gray
  end
  ...
  task :report_totals_to_email do
    ...
    puts "Kanji report email enqueued".gray
  end
  ...
  task :report_totals_in_bear_note do
    ...
    response = %x[open "#{uri}"]
    puts "Bear '#{ENV["TARGET_BEAR_NOTE_TITLE"]}' note updated".gray
  end
end
